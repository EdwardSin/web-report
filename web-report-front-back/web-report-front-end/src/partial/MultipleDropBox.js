...
import '../css/MultipleDropBox.scss';

class Select extends Component {
    constructor(props) {
        super(props);
        this.state = {
            values: props.selectedOptions ? props.selectedOptions : [],
            selectedIndex: -1,
            focusedValue: -1,
            isFocused: false,
            isOpen: false,
    ...
    shouldComponentUpdate(nextProps, nextState, nextContext) {
        if (this.state.values !== nextState.values && this.props.isSelectionChanged) {
            this.props.isSelectionChanged(nextState.values, nextState.selectedIndex);
        }
        return true;
    }
    ...
    onClickOption(e) {
        ...
        this.setState(prevState => {
            ...
            const index = values.indexOf(value);
            const {options} = this.props;
            const selectedIndex = options.map(x => x.value).indexOf(value);
    ...
    renderValues() {
        const {customPlaceholder, options} = this.props;
        const {values} = this.state;
        let prefix = `${values.length} of ${options.length}`;
        if (values.length === options.length) {
            prefix = `All`
        }
        return (
            <div className="placeholder">
                {`${prefix} ${customPlaceholder} 
             selected`}
            </div>
        )
    }
    ...
    render() {
        ...
        return (
            <div
                className={"multipleDropBox select " + className}
                tabIndex="0"
