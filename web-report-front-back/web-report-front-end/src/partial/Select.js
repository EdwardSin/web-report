...
class Select extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    if(this.state.values.length === 0 && this.props.options.length === 0 && nextProps.selectedOptions.length > 0){
      this.setState({
        values: nextProps.selectedOptions
      })
    }
    if(this.state.values !== nextState.values){
      this.props.isSelectionChanged(nextState.values);
    }
    return true;
  }
