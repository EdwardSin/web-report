...
import ServerFilterContainer from "../partial/FilterContainers/ServerFilterContainer";
...
const headers = [{key: 'tms__id', value: 'TMS ID', sortable: true},
    ...
    {key: 'id', value: 'Server Id', sortable: true},
    {key: 'name', value: 'Server Name', sortable: true},
    {key: 'ip', value: 'Server IP', sortable: true},
    {key: 'type', value: 'Server Type', sortable: true},
    {key: 'serial', value: 'Server Serial', sortable: true},
    {key: 'software', value: 'Server Software', sortable: true},
    {key: 'data_usage', value: 'Data Usage', sortable: true}
];
...
class Server extends Component {
        ...
        return (
            <div>
                <OperationContainer {...this.props}
                                    ...
                                    downloadFilename={`server_report_${moment().format('DD_MM_YYYY_HH_mm_ss')}.csv`}
                                    ...
                                    filterContainer={(isOpen) => {
                                        return <ServerFilterContainer {...this.props}
                                                                      filterItems={this.state.filterItems}
                                                                      isOpen={isOpen}/>
                                    }}
                />
                <WebTable {...this.props} isOpen={this.state.isOpen} loading={this.state.loading}
                          totalItems={this.state.totalItems}
    ...
    fetchData = ({limit = 20, page = 1, order_by, tms_name, type, software, display_columns}, callback) => {
        ...
        let software_statement = software != null ? `&software__in=${software}` : '';

        fetch(`/api/v1/server/?${limit_statement}${offset_statement}${order_by_statement}${software_statement}${type_statement}${tms_name_statement}`, {
            method: 'GET',
            headers: {
                ...
                if (this._isMounted) {
                    this.setState({
                        filterItems: {
                            ...
                            software: Object.keys(_.groupBy(res.objects, 'software')).map(x => {
                                return {value: x}
                            })
                        }
                    })
